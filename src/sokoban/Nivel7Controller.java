 ...
 * @author BRAYAN
 */
public class Nivel7Controller implements Initializable {
    ...
    private final int Vcolumna[] = {2, 2, 2, 2, 3, 4, 5, 6, 7, 8, 9, 9, 9, 8, 7, 6, 5, 4, 3, 5, 7};
    private final int Vfila[] = {4, 5, 6, 7, 8, 8, 8, 8, 8, 8, 7, 6, 5, 4, 4, 4, 3, 3, 3, 6, 6};
    ...
    private void pressedNext(MouseEvent event) throws IOException {
        ...
        if (nextLeven == true) {
            try {
                ...
                //carca la scene del finalizacion del juego
                FXMLLoader loader = new FXMLLoader(getClass().getResource("Finalización del Juego.fxml"));
                Parent root = loader.load();

                //obtiene el controlador del la Scene del finalizacion del juego
                FinalizaciónDelJuegoController sceneFController = loader.getController();
                if (pasos == 59) {
                    pasos -= 5;
                ...
                DATOS7 = pasos;
                //pasa los datos
                sceneFController.DatosDeTransferencia(NOMBRE, DATOS1, DATOS2, DATOS3, DATOS4, DATOS5, DATOS6, DATOS7);

                String direccion = System.getProperty("user.dir") + "/src/Sonidos/final.mp3";
                File file = new File(direccion);
                Media sound = new Media(file.toURI().toURL().toString());
                MediaPlayer mediaPlayer = new MediaPlayer(sound);
                mediaPlayer.play();

                //muestra la scena del finalizacion del juego en una nueva ventana  
                Scene scene = new Scene(root);
                scene.getStylesheets().add("/css/CSS.css");
                stage.setScene(scene);
                stage.centerOnScreen();
...
////////caja2///////////////////////////////////////////////////////////////////
    ...
    private boolean BoxWithBox(int columnaBWB, int filaBWB, int NumeroCaja) {
        ...
        switch (NumeroCaja) {
            ...
            case 1:
                if (((columnaBWB == c2) && (filaBWB == f2)) || ((columnaBWB == c3) && (filaBWB == f3))) {
                    return false;
                }
                break;
            case 2:
                if (((columnaBWB == c1) && (filaBWB == f1)) || ((columnaBWB == c3) && (filaBWB == f3))) {
                    return false;
                }
                break;
            case 3:
                if (((columnaBWB == c1) && (filaBWB == f1)) || ((columnaBWB == c2) && (filaBWB == f2))) {
                    return false;
                }
                break;
        }
    ...
    private void HabilitarBtnNext() throws FileNotFoundException {
        ...
        //#1
        if (((c1 == cOb1) && (f1 == fOb1)) || ((c1 == cOb2) && (f1 == fOb2)) || ((c1 == cOb3) && (f1 == fOb3))) {
            c++;
            caja.setImage(verde);
        } else {
            caja.setImage(amarillo);
        }//#2
        if (((c2 == cOb1) && (f2 == fOb1)) || ((c2 == cOb2) && (f2 == fOb2)) || ((c2 == cOb3) && (f2 == fOb3))) {
            c++;
            caja1.setImage(verde);
        } else {
            caja1.setImage(amarillo);
        }//#3
        if (((c3 == cOb1) && (f3 == fOb1)) || ((c3 == cOb2) && (f3 == fOb2)) || ((c3 == cOb3) && (f3 == fOb3))) {
            c++;
    ...
    private void reiniciarAuto(int autoOusuario) throws MalformedURLException {
        ...
        GridPane.setColumnIndex(personaje, 4);
        ...
        try {
            ...
            Logger.getLogger(Nivel7Controller.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    void DatosDeTransferencia(String nombre, int datos1, int datos2, int datos3, int datos4, int datos5, int datos6) {
